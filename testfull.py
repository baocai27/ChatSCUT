...
def main_loop(text_file_path, audio_output_path):
    while True:
        command = input("Enter command (1 to process, q to quit): ").strip()
        if command == '1':
            with open(text_file_path, 'r', encoding='utf-8') as file:
                text = file.read()
            generate_tts_audio(text, audio_output_path)
            stream_audio_to_audio2face(audio_output_path, a2f_url, instance_name)
        elif command.lower() == 'q':
            print("Exiting the loop.")
            break
        else:
            print("Invalid command. Please enter '1' to process or 'q' to quit.")

if __name__ == "__main__":
    TEXT_FILE_PATH = 'C:\\Users\\13157\\Desktop\\AISYS\\llmresponse.txt'
    AUDIO_OUTPUT_PATH = 'C:\\Users\\13157\\Desktop\\AISYS\\temp.wav'
    
    main_loop(TEXT_FILE_PATH, AUDIO_OUTPUT_PATH)
